...
We show that a patchy-reionization-induced effective birefringence term changes how anisotropic cosmic-birefringence constraints should be interpreted, and that finite-width reionization effects can strongly suppress the naive thin-shell estimate.

### introduction 的な一文

The key question is not only whether an effective patchy contribution exists formally, but also how much of the thin-shell signal survives once finite-width reionization is included.
...
The remaining uncertainty is not the numerical stability of the background solver, but whether a more physical $C_L^{\tau\tau}$ normalization can overcome the strong finite-width suppression seen in the present surrogate setup.
...
したがって、`21/22` は thin-shell upper bound としては useful だが、paper claim の正本としてはそのまま使わない方がよい。
...
## `28/29` を踏まえた framing update

`28-required_template_boost.py` で、current best point を observable-level に押し上げるのに必要な
template normalization boost を逆算した。

- `g = 1.4\times10^{-12}\ {\rm GeV}^{-1}` では 1% に届くのに約 `4.66\times10^1` 倍必要
- `g = 4.0\times10^{-12}\ {\rm GeV}^{-1}` でも 1% に届くのに約 `5.70` 倍必要

さらに `29-amplitude_scaling_sensitivity.py` では、

```math
D_{\rm peak} \propto (A/A_{\rm fid})^{p_{\rm amp}} (b/b_{\rm fid})^{p_b}
```

として scaling を強めても、current parameter range の中で得られる最大 boost は
...
- high-`g` benchmark で 1% レベルを目指すこと自体は normalization にまだ依存しうる
- しかし 10% や 100% に届く story は current surrogate setup ではかなり難しい
- paper の本線は、signal detectability の主張よりも
  - formal decomposition
  - finite-width suppression
  - anisotropic-CB constraints should apply to total birefringence power
  に置く方が安全

ということである。

`30-bestcase_scaled_budget.py` でこの感覚を budget 側へ戻すと、
current parameter range の中でかなり optimistic に見積もっても

- `g = 1.4\times10^{-12}\ {\rm GeV}^{-1}` では約 `7.0\times10^{-3}`
- `g = 4.0\times10^{-12}\ {\rm GeV}^{-1}` でも約 `5.7\times10^{-2}`

が current anisotropic-CB limit に対する上限目安となる。

このことから、少なくとも現状の surrogate family の内部では、

- “current limit にかなり近い large signal”

よりも、

- “finite-width suppression is strong enough that even optimistic normalization changes only give sub-limit, at-most-few-percent signals”

という framing の方が今は正直である。

### Updated safe claim

現時点で一番安全な言い方は、

- thin-shell では patchy term が non-negligible に見えうる
- しかし finite-width visibility averaging を入れると old `$m_{\rm best}$` story は大きく suppress される
- physically relevant window は `$m_{\rm res}\sim10^{-29}$` eV 近傍へ移る
- その近傍にも structure は残るが、current surrogate normalization と benchmark couplings の下では signal は still small
- したがって、現時点の strongest claim は “potentially important decomposition and caveat for constraint interpretation” であって、large observable signal の主張ではない

### finer 201-point rerun

`25` を 201 点で rerun したところ、coarse 31-point scan の best point

```math
m/m_{\rm res} = 0.574349
```

は stable ではなく、best point は
...
ただし、observable-level conclusion 自体は変わらない。
...
であり、finer scan にしても current surrogate setup では still `$10^{-3}$` 級である。

このことはむしろ、main uncertainty が

- the exact location of the oscillatory peak

ではなく、

- the physical normalization of the patchy template itself
